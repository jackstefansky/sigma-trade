...
import { useNewsStore, selectUnreadCount, selectCriticalCount } from '@/lib/store/newsStore';
...
export default function AgentSidebar({ agents }: AgentSidebarProps) {
  const firstEnabled = agents.find((a) => a.enabled)?.id ?? null;
  const [activeAgent, setActiveAgent] = useState<AgentId | null>(firstEnabled);
    ...
    <div className="flex flex-col items-center gap-3 py-4 px-2">
      {agents.map((agent) => {
        ...
        const badgeVariant = agent.id === 'news'
          ? (newsCritical > 0 ? 'critical' as const : 'default' as const)
          : agent.badgeVariant;
        ...
        return (
          <div key={agent.id} className="relative group">
            <button
              onClick={() => {
                if (!isDisabled) setActiveAgent(agent.id);
              }}
              disabled={isDisabled}
              ...
              className={cn(
                ...
                !isDisabled && !isActive && 'hover:scale-105 hover:shadow-[0_0_12px_rgba(0,255,136,0.25)] hover:border-accent/40',
            ...
            {isDisabled && (
              <div className="absolute left-full ml-2 top-1/2 -translate-y-1/2 z-10 hidden group-hover:block">
                <div className="bg-bg-panel border border-border-subtle rounded px-2 py-1 whitespace-nowrap">
                  <p className="font-mono text-xs text-gray-500">Coming in Phase 2</p>
                </div>
              </div>
            )}
          </div>
